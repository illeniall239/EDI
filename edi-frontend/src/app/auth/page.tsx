...
export default function Auth() {
  ...
  const redirectedFrom = searchParams?.get('redirectedFrom') || '/workspaces';

  useEffect(() => {
    // Check for error parameter in URL
    const urlError = searchParams?.get('error');
    if (urlError === 'oauth_callback_error') {
      setError('There was a problem signing in with Google. Please try again.');
    } else if (urlError === 'no_code_provided') {
        ...
        <div className="bg-black/40 backdrop-blur-md border border-blue-900/30 rounded-3xl p-8 shadow-2xl">
          ...
          {error && (
            <motion.div
              ...
              animate={{ opacity: 1, y: 0 }}
              className="mb-6 rounded-lg bg-red-900/20 p-4 border border-red-500/30"
            >
          ...
          >
            <button
              ...
              disabled={isLoading}
              className="w-full bg-white/10 hover:bg-white/20 text-white font-medium py-3 px-4 rounded-xl transition-all duration-200 flex items-center justify-center gap-3 mb-6 border border-white/20 disabled:opacity-50 disabled:cursor-not-allowed group shadow-sm backdrop-blur-sm"
            >
