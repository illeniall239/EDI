...
export default function NativeSpreadsheet({ data = [], onCommand, onDataUpdate, onFileUpload, onClearData, isDataEmpty }: NativeSpreadsheetProps) {
    ...
    try {
      ...
      const assistantMessage = {
        ...
        visualization: response.visualization ? {
          ...
          original_query: response.visualization.original_query || inputCommand
        } : undefined
      };
    ...
    try {
      ...
      if (result.success && result.data) {
        ...
        if (onDataUpdate) {
          ...
          // Dispatch dataUpdate event to trigger workspace save
          console.log('📊 Dispatching dataUpdate event for synthetic dataset');
          ...
          console.log('✅ DataUpdate event dispatched with', result.data.length, 'rows');
  ...
  const analyzeChart = async (visualizationPath: string, originalQuery: string, messageId: string) => {
    try {
                            ...
                            <>
                              ...
                              {message.visualization && (
                                <div className="mt-3 p-3 bg-black/40 rounded-lg border border-blue-500/30 backdrop-blur-sm">
                                  ...
                                  {message.visualization.path.endsWith('.html') ? (
                                    <div className="space-y-2">
                                      ...
                                      <div className="flex gap-2">
                                        ...
                                        <button
                                          onClick={() => analyzeChart(message.visualization!.path, message.visualization!.original_query || 'visualization request', message.id)}
                                          disabled={message.isAnalyzing}
                                          className="inline-flex items-center gap-2 px-3 py-2 bg-purple-600 text-white rounded-lg hover:bg-purple-700 disabled:bg-gray-400 transition-colors text-sm font-medium"
                                        >
                                          {message.isAnalyzing ? (
                                  ...
                                  {message.analysis && (
                                    <div className="mt-4 p-4 bg-black/60 rounded-lg border border-purple-500/30 backdrop-blur-sm">
                                      <div className="text-xs font-medium text-purple-300 mb-3 flex items-center gap-2">
                                        <svg className="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                                          <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M9 19v-6a2 2 0 00-2-2H5a2 2 0 00-2 2v6a2 2 0 002 2h2a2 2 0 002-2zm0 0V9a2 2 0 012-2h2a2 2 0 012 2v10m-6 0a2 2 0 002 2h2a2 2 0 002-2m0 0V5a2 2 0 012-2h2a2 2 0 012 2v14a2 2 0 01-2 2h-2a2 2 0 01-2-2z" />
                                        </svg>
                                        Chart Analysis
                                        <span className={`px-2 py-1 text-xs rounded ${
                                          message.analysis.source === 'gemini' ? 'bg-green-500/20 text-green-300' :
                                          message.analysis.source === 'fallback' ? 'bg-yellow-500/20 text-yellow-300' :
                                      ...
                                      <div className="space-y-3 text-sm">
                                        {message.analysis.chart_type && (
                                          <div>
                                            <h4 className="font-medium text-blue-200 mb-1">📊 Chart Type & Purpose</h4>
                                            <p className="text-blue-100/80">{message.analysis.chart_type}</p>
                                          </div>
                                        )}
                                        
                                        {message.analysis.patterns && (
                                          <div>
                                            <h4 className="font-medium text-blue-200 mb-1">📈 Key Patterns & Trends</h4>
                                            <p className="text-blue-100/80">{message.analysis.patterns}</p>
                                          </div>
                                        )}
                                        
                                        {message.analysis.insights && (
                                          <div>
                                            <h4 className="font-medium text-blue-200 mb-1">💡 Actionable Insights</h4>
                                            <p className="text-blue-100/80">{message.analysis.insights}</p>
                                          </div>
                                        )}
                                      </div>
                                    </div>
                                  ...
                                  {message.analysisError && (
                                    <div className="mt-4 p-3 bg-red-500/20 border border-red-500/30 rounded-lg">
                                      <div className="text-sm text-red-300">{message.analysisError}</div>
                                    </div>
                                  )}
                                </div>
                              )}
